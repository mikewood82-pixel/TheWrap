import { Link } from 'react-router-dom'
import { Play } from 'lucide-react'
import { wrapline } from '../data/wrapline'

export default function WraplinePage() {
  const [hero, ...rest] = wrapline

  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="mb-10">
        <div className="text-brand-terracotta text-xs uppercase tracking-widest font-medium mb-2">Wrapline</div>
        <h1 className="font-serif text-4xl font-bold mb-3">Wrapline</h1>
        <p className="text-brand-dark/60 text-lg max-w-xl">
          The Wrap's news-magazine segment — longer-form video on the people and
          stories behind HR tech. New segments drop when there's one worth telling.
        </p>
      </div>

      {/* Latest segment — full embed */}
      {hero && (
        <div className="mb-10">
          <div className="text-brand-dark/40 text-xs uppercase tracking-widest font-medium mb-2">
            Latest Segment · {hero.date}
          </div>
          <h2 className="font-serif text-2xl font-bold mb-4">{hero.title}</h2>
          <div className="aspect-video w-full rounded-xl overflow-hidden bg-brand-dark mb-3">
            <iframe
              src={`https://www.youtube.com/embed/${hero.youtubeId}`}
              title={`Wrapline — ${hero.title}`}
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              className="w-full h-full"
            />
          </div>
          {hero.description && (
            <p className="text-brand-dark/70 leading-relaxed">{hero.description}</p>
          )}
          {hero.newsletterSlug && (
            <p className="mt-3">
              <Link
                to={`/newsletter/${hero.newsletterSlug}`}
                className="text-brand-terracotta text-sm font-medium hover:underline"
              >
                Read the written edition →
              </Link>
            </p>
          )}
        </div>
      )}

      {/* Past segments */}
      {rest.length > 0 && (
        <div className="mb-12">
          <h2 className="font-serif text-xl font-bold mb-4">Past Segments</h2>
          <ul className="space-y-3">
            {rest.map((seg) => (
              <li
                key={seg.number}
                className="bg-brand-surface border border-brand-border rounded-lg p-4 hover:border-brand-terracotta/40 transition-colors"
              >
                <a
                  href={`https://www.youtube.com/watch?v=${seg.youtubeId}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-start gap-4"
                >
                  <div className="shrink-0 w-10 h-10 bg-brand-terracotta/10 text-brand-terracotta rounded-full flex items-center justify-center">
                    <Play size={18} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-brand-dark/40 text-xs uppercase tracking-widest font-medium mb-1">
                      {seg.date}
                    </div>
                    <div className="font-serif font-bold mb-1">{seg.title}</div>
                    {seg.description && (
                      <div className="text-brand-dark/60 text-sm">{seg.description}</div>
                    )}
                  </div>
                </a>
              </li>
            ))}
          </ul>
        </div>
      )}
    </div>
  )
}
